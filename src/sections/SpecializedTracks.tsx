...
import SiadArrow from '@/assets/saidArrow.svg'
    ...
    <section>
        <Wrapper>
            ...
            <div>
                ...
                <p className='mt-3 text-lg text-slate-600 leading-[30px] mx max-w-screen-sm'>
            ...
            <div  className='mt-10 flex flex-col-reverse md:flex-row gap-x-6 gap-y-8'>
                ...
                <div className='shadow-xl self-start sticky top-28 basis-8/12 rounded-xl border border-slate-200 py-8 px-8'>
                    <h4 className='text-real-700 text-lg font-medium'>Specialized Program</h4>
                    ...
                    <Image className='text-teal-700 mb-1 w-5 h-5' src={SiadArrow} alt='sideArrow' /></button>

                <div className='flex gap-4 mt-8 flex-col md:flex-row'>
                    <QuarterBox title={selectedItemData?.quarters[0].header} paragraph={selectedItemData?.quarters[0].description} num={selectedItemData?.quarters[0].number} haveBorder={false} />
                    <QuarterBox title={selectedItemData?.quarters[1].header}  paragraph={selectedItemData?.quarters[1].description}  num={selectedItemData?.quarters[1].number} haveBorder={false} />
                </div>
                ...
                <div className='px-4 py-6 space-y-4 basis-4/12 '>
                    {
                        data.map((item,index)=>{
                            return(
                                <>
                                <div onClick={()=>setSelectedItem(item.slug)} key={index} className='flex gap-x-3 items-center cursor-pointer'>
                                    <div>
                                        <div className='w-40 h-24  object-cover rounded-md'> <Image src={item.image} alt={item.header} /> </div>
                                    </div>
                                    <div>
                                        <h4 className='text-teal-700 text-14 font-medium'>Specialized Program</h4>
                                        <h3 className='text-16 xs:text-18 font-bold'>{item.header}</h3>
                                    </div>
                                </div>
